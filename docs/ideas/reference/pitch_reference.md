...
ナイスピッチミスの際もストライクゾーンを狙っていた場合には必ずストライクゾーン内に、ボールゾーンを狙っていた場合は必ずボールゾーン内に着弾点が入る特殊な仕様はストライクが先行してるカウントでは上手く活用したい。(変化球情報画面、および上方向の第２球種には表示しない)  
...
- Dスライダー: 2026追加候補。記事上はカーブ系。現行UIでは暫定的に右下系として扱う。
- ファストチェンジ: 2026追加候補。記事上はシンカー系。現行UIでは暫定的に左下系として扱う。
- シンキングスプリット: 2026追加候補。記事上はシンカー系。現行UIでは暫定的に左下系として扱う。
- 今後、変化角度や細かい方向表示に対応する場合は、より詳細な角度ベースの設計を検討する。
