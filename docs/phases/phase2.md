...
- Prompt5-6 URL・状態管理整理が進行中
- 管理コードの画面表示・検索・export連携への本格展開が未実装

一方で、安定化基盤として core regression test、一時 SQLite DB、`npm run verify:all`、GitHub Actions、`codex/staging` Ruleset、required check `verify-all` は整備済みである。
...
- `npm run verify:all` による構文確認、DB 診断、core regression test、差分 check の一括実行
...
- 6.4-1 URL・状態管理の設計調査は完了
- 6.4-2 schools pure state 分離は完了
- 6.4-3 players pure state 分離は完了
- 6.4-4 frontend静的解析・削除安全基盤が次作業
- 6.4-5 schools非同期競合対策を実施予定
- 6.4-6 History API検証・最小整理を実施予定
- 6.4-7 最終確認・docs同期を実施予定
- 現在の URL 同期は直ちに廃止しない
- legacy query との互換を維持する
