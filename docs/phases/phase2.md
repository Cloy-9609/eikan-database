...
- 6.5-1 保全・export・import・bundle仕様確定 完了
- 6.5-2 DB検査基盤強化 次の作業
- 6.5-3 SQLite backup 予定
- 6.5-4 SQLite restore 予定
- 6.5-5 version付きJSON export 予定
- 6.5-6 import dry-run 予定
- 6.5-7 空DBへの完全import 予定
- 6.5-8 データ管理画面 予定
- 6.5-9 最終検証・docs同期 予定
...
SQLite DBはGit管理外であり、source ZIPとは別に保全する必要がある。6.5-1では [DB backup / restore 設計](../design/database_backup_restore.md)、[data export / import 設計](../design/data_export_import.md)、[転生選手 bundle・作品管理 概念設計](../design/reincarnated_player_bundle_design.md) を追加した。次の作業は 6.5-2 DB検査基盤強化である。
