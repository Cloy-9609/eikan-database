...
### 6.3 snapshot・検索・年度進行の自動回帰テスト

状態: 次の作業

- snapshot 表示時点選択
- players 検索・能力範囲検索・sort
- 学校年度進行と undo
- 上記の回帰確認を自動化する
...
- URL同期を廃止するとは確定していない
- 現在のURL同期を維持しながら、責務整理・テスト・必要に応じた分割を行う予定
- `docs/design/state_management.md` の大規模更新はこの作業で扱う
...
- OCR本体MVP、DB保全、回帰テスト、状態管理、管理コード本格展開の残量を確認する
...
- DB backup / restore、export / import、自動回帰テストの整備方針が明確になっている
