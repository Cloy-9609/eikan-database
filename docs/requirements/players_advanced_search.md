...
- 球速 `velocity` と弾道 `trajectory` はランクなしとして扱い、数値範囲のみ指定する。
...
backendでは、frontendから送信された能力keyをSQLカラム名へ直接埋め込まず、safe whitelist mapで許可済みkeyから `players.<column>` へ変換する。min/max値はSQL parameterとして渡す。DB schema変更、migration、index追加はPrompt5-3では不要とする。
