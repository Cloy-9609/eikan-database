...
注: URL・状態管理については Phase 6.4 完了後の `frontend/js/state/playerSearchState.mjs` と `docs/design/state_management.md` が source of truth である。本書内の関数名は 2026年7月時点の最終実装に合わせて更新している。
...
| 関数 | 役割 |
| --- | --- |
| `createDefaultPlayerSearchState()` | 検索状態の初期値。 |
...
`PLAYER_SEARCH_QUERY_KEYS` に列挙されたkeyがURL正規化時に削除対象となる。2026年7月時点では `snapshot_label` も含まれ、snapshot表示時点選択のURL同期対象になっている。
...
- クリア: `createDefaultPlayerSearchState()` をformとURLへ反映し、再取得。sortは更新日時降順へ戻る。
...
- PC表示では、能力要約が通常行の全列幅を使用する。
- 通常行は有効なtable markupを維持し、能力要約用の新しい `tr` を追加しない。
- 通常行内は `colspan` したcellと内部gridで構成し、上段に既存6項目（選手名、学校名、ポジション、総合星、学年 / 状態、入学年）を既存table headerと視覚的に対応する順序で配置する。
- 下段の能力要約は内部gridの左端から右端まで横断し、通常行の右側に大きな未使用領域を残さない。
- 通常のPC幅では、野手7能力（弾道、ミート、パワー、走力、肩力、守備力、捕球）を原則1行で表示する。
- 能力要約は1能力につき1つのフラットな表示要素とし、能力名、区切り、数値を同一要素内に横並びで表示する。
- 能力値だけを内側のpillやbadgeで囲まず、ランク色は能力項目全体へ適用する。
- 641px付近など幅が不足する場合に限り、gapやpaddingを抑えたうえで自然な折り返しを許容する。
- 絶対配置、負のmargin、隣接cellへのはみ出し、`overflow: visible` による疑似的な全幅化、JavaScriptでの座標・横幅計算は使用しない。
...
- アコーディオン詳細を開いている選手の通常行では、一覧能力要約を非表示にし、閉じたら保存中の投手/野手modeの要約を再表示する。
- 新たな横スクロールを発生させない。
...
- アコーディオン詳細内の通常能力値にも、一覧能力要約と同じ判定に基づくランク色、球速固定色、弾道色を控えめに適用する。
- 特殊能力、サブポジション、総合星、登録時点、タイプ、メモなど、通常能力ではない値には能力ランク色を付けない。
...
- `PLAYER_SEARCH_QUERY_KEYS`、`readPlayerSearchStateFromParams()`、`normalizePlayerSearchState()`、`buildPlayerListParams()`、`buildCanonicalPlayerSearchParams()`、page adapter の `readSearchStateFromForm()` / `applySearchStateToForm()` のすべてに条件追加が必要。
...
- 1回の検索につき能力条件は1件のみ。
- 能力範囲検索は独立したaccordionにはせず、「状態・表示」内の通常能力subgroupとして常時表示する。
- 検索機能、URL、API query、backend validation、SQL条件はPrompt5-3のまま維持する。
...
Prompt5-4では、既存のsort queryへ通常能力値によるsortを追加する。対象はPrompt5-3の通常能力範囲検索と同じ10項目で、一覧表示・能力範囲検索と同じ最新相当snapshotの能力値を使う。UI上は「状態・表示」から分離した独立した並べ替えpanelに置き、並べ替え基準selectと昇順／降順buttonで操作する。
...
各能力は並べ替え基準select内に1回だけ表示し、方向は昇順／降順buttonで管理する。高い順は `sort_order=desc`、低い順は `sort_order=asc` とし、弾道も同じ規則で4→1 / 1→4に並べる。select内は「基本の並び順」「能力値」のoptgroupを維持する。
