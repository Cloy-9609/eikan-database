...
- 上段には既存6項目（選手名、学校名、ポジション、総合星、学年 / 状態、入学年）を配置し、下段の能力要約は左端から右端まで横断する。
- 通常行は有効なtable markupと内部gridで構成し、学校名や他列が能力要約の縦中央に浮いて見えないようにする。
...
- 新たな横スクロールを発生させない。
- 通常のPC幅では野手7能力を1行表示し、641px付近で必要な場合に限り折り返しを許容する。
- 行高が過度に大きくならないようにする。
