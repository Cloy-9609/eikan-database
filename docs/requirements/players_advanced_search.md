...
`0`、null、空文字はランクなし/未設定相当として扱うのが自然である。現行 `getAbilityRankForValue()` は数値がどのrank groupにも入らなければnullを返す。

### 5.3 null / 未設定値の扱い

- DB上、通常能力はnullable。未入力ならnullになり得る。
- 入力UIでは空欄は空文字として送られ、`parseOptionalInteger()` によりnull相当になると推測される。
- `total_stars` は0を未設定sentinelとして扱うが、通常能力の0については現行要件上「0〜100」とされており、未設定とは別の値になり得る。ただしG〜Sランクは1以上から始まるため、0の表示・検索仕様は未確定。
- players一覧の既存 `formatStatValue()` はnull/空なら `-` を表示する。0は `0` と表示される。
...
2行目の能力要約は、通常行内に入れる方法と、通常行の直後に新しい要約行を入れる方法がある。

推奨は「通常行内の選手名セルまたは全列colspan行に能力要約を追加する」よりも、「通常行のセル構成を保ったまま、選手名セル配下または新規専用セル相当に要約ブロックを置く」方式である。理由は、既存アコーディオン行がすでに通常行直後の `tr` を使っており、新たな2行目 `tr` を追加すると、mobileカード境界、expanded時の隣接関係、`previousElementSibling` に依存する `setAccordionExpanded()` の挙動を調整する必要があるためである。

ただしPCで明確な「1行目/2行目」に見せたい場合は、通常行を `td colspan` で再設計するより、既存tableを維持しつつ名前セルや別列内に `.players-ability-summary` を2段表示する方が低リスクである。

### 6.2 既存アコーディオンとの競合

注意点は以下。

- `setAccordionExpanded()` は `detailRow.previousElementSibling` を通常行として扱う。通常行と詳細行の間に能力要約 `tr` を挟むとexpanded class付与対象がずれる。
- `PLAYERS_TABLE_COLUMN_COUNT` は6。列追加やcolspan構成変更時は更新が必要。
- mobileでは通常行がカード、アコーディオン行がカード下部として接続されるため、要約行を別trにするとカードが3分割される可能性がある。

### 6.3 表示項目案

PCでの推奨能力要約。

- 投手: 球速、コン、スタ、主要変化球数または最大変化量。ただし変化球は一覧APIにないため、初期段階では球速・コン・スタに限定する。
- 野手: 弾道、ミート、パワー、走力、肩力、守備、捕球。
- 総合星は既存列にあるため要約からは重複させない。

スマートフォンでの推奨能力要約。

- 投手: 球速、コン、スタの3項目まで。
- 野手: ミート、パワー、走力、守備の4項目程度。弾道・肩力・捕球は省略候補。
- 詳細は既存アコーディオンで確認する。

投手兼野手相当の選手。

- メインポジションが投手なら、一覧要約は投手能力を優先する。
- PCでは「野手: ミ パ 走」程度を補助行として追加する余地はあるが、初期実装では詰め込みすぎを避ける。
- アコーディオンの既存toggleで野手能力を確認できるため、一覧は投手能力優先でよい。

能力未登録時。

- 全対象値がnull/空なら「能力未登録」または `—` を表示。
- 一部だけ未登録なら項目ごとに `—` 表示。
- 0は未設定扱いにするか有効値扱いにするか要確認。現行 `formatStatValue()` に合わせるなら0は `0` 表示。

### 6.4 横スクロール・高さ・アクセシビリティ

- mobileは横スクロールを避け、能力chipを折り返し可能なflex/gridにする。
- 1選手カードの高さが過度に増えないよう、mobileでは項目数を絞り、2行以内を目標にする。
- 能力略称だけでなく、`aria-label` や `title` に正式名称を含める。
- 色だけでランクを表現しない。文字ランク・数値を併記する。
- toggle buttonのクリック範囲とリンククリック範囲を維持する。
...
| 領域 | 規模 | リスク | 理由 |
| --- | --- | --- | --- |
| 一覧2行表示 | 小〜中規模 | 低〜中 | 一覧APIに能力値が既にある。主なリスクはtable/mobile/accordionとの競合。 |
...
### Prompt5-2: players一覧の2行表示と能力要約

- 目的: 最新snapshot前提で、一覧に投手/野手能力要約を表示する。
- 変更候補ファイル: `frontend/js/pages/players.js`, `frontend/css/players.css`。
- frontend変更: あり。
- backend/API変更: なし。
- DB変更: なし。
- リスク: 低〜中。mobileカードとアコーディオン隣接構造に注意。
- 規模感: 小〜中。
- 依存: なし。
- 実ブラウザ確認: PC幅、640px前後、スマホ幅、アコーディオン開閉、投手toggle。
- 自動merge可否: 低リスクなら可。ただしUI目視確認前提ならPRで停止も可。
- 完了条件: 能力未登録・投手・野手で要約表示が崩れない。既存検索・sort・URLが変わらない。
...
1. PC一覧の2行目には、投手は「球速・コントロール・スタミナ」、野手は「弾道・ミート・パワー・走力・肩力・守備・捕球」を表示する方針でよいですか。
2. スマートフォンでは、投手は3項目、野手は「ミート・パワー・走力・守備」程度に絞り、残りはアコーディオン確認でよいですか。
3. 投手兼野手相当の選手は、メインポジションが投手なら一覧要約では投手能力を優先し、野手能力はアコーディオンで確認する方針でよいですか。
4. 複数能力条件は初期実装ではAND検索のみでよいですか。OR検索は将来タスクでよいですか。
5. snapshot指定時に、その時点が未登録の選手は一覧から除外する仕様でよいですか。それとも下部へ回して最新snapshotで代替表示したいですか。
6. 「現在表示中snapshot」は、初期状態では現行最新相当snapshot、snapshot select選択後はその選択時点、という定義でよいですか。
7. 詳細条件UIは、現行レイアウトに馴染むアコーディオン方式を優先してよいですか。sticky asideは後回しでよいですか。
8. URL反映は最終的に完全廃止したいですか。それとも検索条件共有と戻る/進むのため維持したいですか。
9. 通常能力値の0は有効値として検索・表示しますか。それとも未設定扱いに寄せますか。
10. 実DBにlegacy `admission` snapshotが残っている可能性を考慮し、入学時検索で `entrance` 以外も対象にする必要がありますか。
