...
| stale failure ignored while latest busy | request A 開始後に request B を開始し、B 処理中に A 失敗。 | A は error message を表示せず、A は B の busy 状態を解除しない。 |
| rapid popstate | back / forward 相当の URL 変更を短時間に連続実行。 | 最後の URL 条件に対応する結果だけが描画される。 |
